...
### Fixed
- Docker-compose port mappings with shell variable references (`${HOST_PORT:-80}:80`) no longer crash with "Invalid port: -80"
- Port range mappings (`3000-3005:80-85`) return only the range start instead of expanding into many duplicate entries
